...
import type { BookingDTO, UpdateBookingDTO } from "../../queries/bookingSchemas";
...
const mapBookingDTOToBookingFormItem = (booking: BookingDTO): IBookingFormItem => {
  return {
...
export const mapUpdateBookingShemaTypeToUpdateBookingDTO = (payload: BookingShemaType): UpdateBookingDTO => {
  const [hour1, minute1] = payload.timeSlot.start.split(":").map(Number);
  const dt1 = DateTime.now().set({ hour: hour1, minute: minute1, second: 0, millisecond: 0 });

  const [hour2, minute2] = payload.timeSlot.end.split(":").map(Number);
  const dt2 = DateTime.now().set({ hour: hour2, minute: minute2, second: 0, millisecond: 0 });
...
export const EditBookingAdminPage = () => {
  ...
  const handleEdit = (data: BookingShemaType) => {
    update.mutate({
      ...
      body: mapUpdateBookingShemaTypeToUpdateBookingDTO(data),
      headers: {
        authorization: accessToken,
      }
    })
